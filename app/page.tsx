...
export default function Home() {
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        setMessages([...newMessage, ...response]);
    };
    ...
    return (
        <div className="h-screen flex flex-col w-full max-w-md py-24 mx-auto stretch ">
            ...
            <form onSubmit={handleSubmit}>
                <input type="text" name="message" className="fixed bottom-0 p-3 border-2 rounded-lg mb-8 w-full max-w-md" placeholder="テキストを入力してください" />
            </form>
        </div>
    );
}
